...
export const metadata: Metadata = {
  title: "下载",
  description: "下载适用于 macOS 或 Windows 的最新 Madora 稳定版本。",
};

const assurances = [
  { icon: FolderSimpleLock, title: "默认保存在本地", text: "Markdown 工作区保留在你的电脑中，始终由你掌控。" },
  { icon: ShieldCheck, title: "经过验证的版本", text: "每个安装包都提供由稳定发布通道公布的 SHA-256 校验值。" },
  { icon: ArrowsClockwise, title: "自主决定更新", text: "Madora 会检查签名更新，但不会在后台强制安装。" },
];
  ...
  return (
    <div className="download-page">
      <section className="container download-hero page-load-reveal">
        <p className="eyebrow">下载 Madora</p>
      ...
      <section className="container download-assurances scroll-reveal" aria-label="为什么下载 Madora">
        {assurances.map((item) => {
      ...
      <section className="download-install-section">
        ...
        <div className="container download-install-grid scroll-reveal">
          <article>
            ...
            <div><p className="eyebrow">macOS</p><h3>打开 DMG，将 Madora 移动到 Applications。</h3><p>M1 或更新机型请选择 Apple Silicon；较早的设备仍可使用 Intel 版本。</p></div>
          </article>
          <article>
            <span className="download-install-icon"><WindowsLogo aria-hidden size={24} weight="fill" /></span>
            <div><p className="eyebrow">Windows</p><h3>运行安装程序并按照指引完成安装。</h3><p>当前 Windows 版本支持 x64 系统，并随附标准 Madora 更新程序。</p></div>
          </article>
        </div>
      </section>
    </div>
