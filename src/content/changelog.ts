export const changelogEntries = [
  {
    ...
    paragraphs: [
      "Markune 现在可以直接与 Figma 集成。你可以将 Figma 文件关联到任意任务，并直接在工作区内预览设计，无需离开当前页面。",
      "附加 Figma 文件后，任务会显示实时预览缩略图，并在设计发生变化时自动更新。团队成员既可以打开完整的 Figma 文件，也可以直接在任务详情中审阅预览，无需再把链接粘贴到评论中，或频繁切换工具查看设计进度。",
      "该集成同样支持 Figma 的分支功能。如果设计团队使用分支进行迭代，Markune 会在预览旁显示分支名称，确保审阅者始终清楚当前查看的是哪个版本。",
  ...
  },
  {
    ...
    paragraphs: [
      "所有 Business 方案用户现已可以使用自定义自动化规则。你可以直接在 Markune 中构建自己的 if/then 工作流，无需编写代码，也无需借助第三方工具。",
      "你可以根据状态变化、截止日期、负责人更新、优先级调整等条件触发操作。例如，当任务被标记为“已完成”时，Markune 可以自动通知项目负责人并将任务移至“审阅”列；当距离截止时间不足 48 小时且任务仍为“进行中”时，负责人会收到提醒；在指定项目中创建新任务后，也可以立即自动分配给合适的成员。",
